    ...
    { 
      ...
      render: (val: string) => (
        <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${
          val === 'banner' ? 'bg-navy-50 text-navy-600' :
    ...
    { 
      ...
      render: (val: boolean, row: any) => (
        <button 
           ...
           className={`group flex items-center gap-2 px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
           val ? 'bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white' : 'bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white'
        }`}>
        ...
        <div className="space-y-8 p-4">
            <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-navy-400">Content Type</label>
                        ...
                        {['banner', 'popup'].map(t => (
                            <button 
                                ...
                                className={`py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border-2 transition-all ${
                                    formData.type === t ? 'bg-navy-950 text-gold-500 border-navy-950 shadow-xl' : 'bg-gray-50 text-gray-400 border-transparent hover:border-gray-100'
                                }`}
                            >{t}</button>
                        ))}
                    </div>
            ...
            <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-navy-400">Content Body (Markdown Supported)</label>
                <textarea 
