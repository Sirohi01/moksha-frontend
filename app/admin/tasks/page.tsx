              ...
              {task.assignedTo.slice(0, 2).map((assignment, index) => (
                <div key={index} className="text-sm">
                  ...
                  <div className="flex space-x-1 mt-1">
                    <span className={`px-2 py-1 rounded-full text-xs ${assignment.status === 'accepted' ? 'bg-green-100 text-green-800' :
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs ${assignment.adminApprovalStatus === 'approved' ? 'bg-green-100 text-green-800' :
                  ...
                  <div className="space-y-2">
                    {selectedTask.assignedTo.map((assignment, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${assignment.status === 'accepted' ? 'bg-green-100 text-green-800' :
                            assignment.status === 'rejected' ? 'bg-red-100 text-red-800' :
                              'bg-yellow-100 text-yellow-800'
                          }`}>
                          {assignment.status}
                    ...
                    <div key={index} className="bg-gray-50 p-6 rounded-xl border">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="flex flex-col items-end space-y-2">
                          ...
                          <span className={`px-3 py-1 rounded-full text-xs font-medium ${assignment.adminApprovalStatus === 'approved' ? 'bg-green-100 text-green-800' :
                              assignment.adminApprovalStatus === 'rejected' ? 'bg-red-100 text-red-800' :
                                'bg-yellow-100 text-yellow-800'
                            }`}>
                          ...
                          <div className="flex space-x-2">
                            {['pending', 'accepted', 'rejected', 'in_progress', 'completed'].map((status) => (
                              <button
                                ...
                                className={`px-3 py-1 text-xs rounded-lg transition-colors ${assignment.status === status
                          ...
                          <div className="flex space-x-2">
                            {['pending', 'approved', 'rejected'].map((approvalStatus) => (
                              <button
                                ...
                                className={`px-3 py-1 text-xs rounded-lg transition-colors ${(assignment.adminApprovalStatus || 'pending') === approvalStatus
                                    ? 'bg-blue-500 text-white'
                                    : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                                  }`}
                              >
              ...
              <div className="space-y-3">
                {['draft', 'assigned', 'in_progress', 'completed', 'cancelled', 'overdue'].map((status) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 rounded-lg border transition-colors ${selectedTask.overallStatus === status
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
            <div className="p-6">
              <div className="flex items-center mb-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center mr-4 ${confirmAction.type === 'success' ? 'bg-green-100' :
