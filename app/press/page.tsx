                ...
                <Container>
                    <div className="max-w-3xl text-left">
                        <div className="inline-block px-4 py-1.5 rounded-full bg-[#7ab800]/10 border border-[#7ab800]/20 mb-6">
                            <p className="text-[#7ab800] font-black text-[10px] uppercase tracking-[0.4em] leading-none">{pressConfig.hero.badge}</p>
                        </div>
                        <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tighter leading-[0.85] mb-8">{pressConfig.hero.title} <br /><span className="text-[#7ab800]">{pressConfig.hero.subtitle}</span></h1>
                        <p className="text-stone-400 text-lg md:text-xl font-medium leading-relaxed">
                            {pressConfig.hero.description}
                        </p>
                    </div>
                </Container>
            </section>
            ...
            <section className="py-20">
                <Container>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {pressConfig.pressCoverage.items.map((item, i) => (
                            <div key={i} className="bg-white p-10 rounded-[3rem] border border-stone-100 shadow-sm hover:translate-y-[-4px] transition-all group overflow-hidden relative text-left">
                                ...
                                <button className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-[#7ab800] hover:text-[#5b8a00] transition-colors">
                                    {pressConfig.pressCoverage.readButton} <ExternalLink size={14} />
                                </button>
                            </div>
                        ))}
                    </div>
                ...
                <Container>
                    <div className="bg-white rounded-[4rem] p-12 md:p-20 border border-stone-200 shadow-xl text-left">
                        <div className="flex items-center gap-4 mb-10">
                            ...
                            <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tighter text-stone-800 leading-[0.85]">{pressConfig.assetLibrary.title}</h2>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            {pressConfig.assetLibrary.assets.map((asset, i) => (
            ...
            <section className="py-24 bg-white">
                <Container size="md">
                    <div className="text-center p-12 md:p-20 bg-stone-900 rounded-[4rem] text-white">
                        <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tighter leading-[0.85] mb-8">{pressConfig.mediaContact.title} <span className="text-[#7ab800]">{pressConfig.mediaContact.subtitle}</span> INQUIRIES</h2>
                        <p className="text-stone-400 font-medium text-lg leading-relaxed mb-10">
                            {pressConfig.mediaContact.description}
                        </p>
                        <div className="flex flex-col sm:flex-row gap-8 justify-center items-center">
                            {pressConfig.mediaContact.contacts.map((contact, i) => {
