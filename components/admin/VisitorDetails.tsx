  ...
  if (activities.length === 0) {
    return (
      <div className="bg-white dark:bg-[#0B1120] p-12 rounded-[2.5rem] text-center shadow-xl border border-gray-100 dark:border-white/5 relative overflow-hidden">
        ...
        <p className="text-gray-500 dark:text-navy-400 max-w-sm mx-auto mb-8 text-sm">No activity logs found for this IP.</p>
        <Link 
        ...
        <div className="p-8 md:p-12 relative z-10">
          <div className="flex flex-col lg:flex-row gap-12 items-center lg:items-start">
            ...
            <div className="flex flex-col items-center lg:items-start text-center lg:text-left min-w-[240px]">
              ...
              <div className="flex items-center gap-2 text-[11px] font-bold text-navy-400 italic">
            ...
            <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-6 w-full">
               ...
               <div className="p-6 bg-gray-50 dark:bg-white/5 rounded-3xl border border-gray-100 dark:border-white/5 group hover:border-gold-500/20 transition-all">
                  ...
                  <p className="text-sm font-black text-navy-950 dark:text-white uppercase leading-tight mt-2">
                    ...
                    <br />
                    <span className="text-navy-400 font-bold">{formatDate(activities[activities.length - 1].startTime, { hour: '2-digit', minute: '2-digit' })}</span>
                  </p>
               </div>
               <div className="p-6 bg-gray-50 dark:bg-white/5 rounded-3xl border border-gray-100 dark:border-white/5 group hover:border-gold-500/20 transition-all">
                  <div className="flex items-center gap-2 text-gold-600 font-black text-[9px] uppercase tracking-widest mb-2 opacity-50">
                  ...
                  <p className="text-[10px] text-navy-400 font-mono mt-1 opacity-50 truncate" title={latestSession.userAgent}>
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-10">
                    <div className="space-y-4 flex-1">
                      ...
                      <div className="flex flex-wrap gap-3">
                        <div className="flex items-center gap-2 px-4 py-2 bg-gray-50 dark:bg-white/5 rounded-2xl text-[11px] font-bold text-navy-500 dark:text-navy-400 border border-transparent hover:border-gray-100 transition-all">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                          {session.events.map((event, eIdx) => (
                            <div key={eIdx} className="group/event bg-gray-50 dark:bg-white/5 p-5 rounded-3xl border border-transparent hover:border-gold-500/30 hover:bg-white dark:hover:bg-navy-900 transition-all duration-300">
                              <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transform group-hover/event:rotate-6 transition-all ${
                                  event.type === 'click' ? 'bg-gold-500 text-navy-950' : 'bg-navy-900 text-gold-500'
                                }`}>
                                  {event.type === 'click' ? <MousePointer2 className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center justify-between mb-1 gap-2">
                                    <span className={`text-[8px] font-black uppercase tracking-[0.1em] ${event.type === 'click' ? 'text-gold-600' : 'text-navy-400'}`}>
                                      {event.type} detected
                                    </span>
